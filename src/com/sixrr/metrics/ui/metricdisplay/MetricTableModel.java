...
class MetricTableModel extends AbstractTableModel {
    ...
    public int getRowCount() {
        if(measuredObjects.length>0)
            return hasSummaryRows() ? measuredObjects.length + 6 : measuredObjects.length + 4;
        return 0;
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        ...
        if (hasSummaryRows()) {
            if (rowIndex==measuredObjects.length+2){
                if (permutedColumn == 0)
                    return "Energy Insight";
                final MetricInstance metricInstance = metricsInstances[permutedColumn - 1];
                String s=(String) EnergyInsightsConfig.getInstance().getValueOf(metricInstance.getMetric().getAbbreviation());
                return s.split(":")[0];
            }
            if(rowIndex==measuredObjects.length+3){
                if(permutedColumn==0)
                    return "Impact";
                final MetricInstance metricInstance = metricsInstances[permutedColumn - 1];
                String s=(String) EnergyInsightsConfig.getInstance().getValueOf(metricInstance.getMetric().getAbbreviation());
            ...
            if(rowIndex==measuredObjects.length+4){
                if(permutedColumn==0)
                    return "Change Ratio";
                final MetricInstance metricInstance = metricsInstances[permutedColumn - 1];
                String s=(String) EnergyInsightsConfig.getInstance().getValueOf(metricInstance.getMetric().getAbbreviation());
            ...
            if(rowIndex==measuredObjects.length+5){
                if(permutedColumn==0)
                    return "Overall Energy";
                if(columnIndex<(columnPermutation.length-1))
                    return "";
                return EnergyInsightsConfig.getInstance().getOverallEnergy();
            }
    ...
    private void sort() {
        ...
        if (permutedColumn == 0) {
            ...
            for (int i = 0; i < rowPermutation.length; i++) {
                ...
                String formattedValue = FormatUtils.formatValue(metricInstance.getMetric(), value);
                if (formattedValue.indexOf((int) ',') >= 0) {
                    formattedValue = '"' + formattedValue + '"';
                }
                tempArray[i] = Pair.create(Integer.valueOf(i), formattedValue);
            }
        }
